...
import { Button, Col, Empty, Input, Row, Skeleton, Table } from "antd";
...
function ContractsList(props: Props) {
  const router = useRouter();
  const [contract, setContract] = useState(props.initialContracts);
  ...
  useEffect(() => {
    ...
    loadContracts(newPage, newSize, newSearch, newSort);
  ...
  function handleChangePage(newPage: number, newPageSize: number) {
    ...
    loadContracts(newPage, newPageSize);
  }
  ...
  contract.map((user: Contract) => {
    const { id, _id, name, number, customer, status, email, created_at } = user;
      ...
      action: (
        <div className="min-w-[150px] text-end -m-2">
          <Link
            ...
            href={`/admin/contracts/customer/detail/${_id}`}
          >
