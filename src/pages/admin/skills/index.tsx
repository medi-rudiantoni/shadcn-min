...
import { getSkills, removeSkill } from "@/functions/skill";
...
function SkillList() {
  ...
  skill.map((user: User) => {
    const { _id, id, name, role, img, status, email, createdAt } = user;
    ...
    return usersTableData.push({
      ...
      action: (
        <div className="min-w-[150px] text-end -m-2">
          <Link className="inline-block m-2" href={`/admin/skills/${_id}`}>
            <Button className="bg-primary text-white">
              <UilEye className="w-4" /> View
            </Button>
          </Link>
          <Link className="inline-block m-2" href={`/admin/skills/edit/${_id}`}>
  ...
  return (
    <>
      ...
      <div className="min-h-[715px] lg:min-h-[580px] flex-1 h-auto px-8 xl:px-[15px] pb-[30px] bg-transparent">
        <Row gutter={15}>
          <Col xs={24} className="mb-[25px]">
            <Button type="primary" className="mb-3">
              <Link href="/admin/skills/add">Add Skill</Link>
            </Button>
        ...
        >
          <div>Are you sure want to Delete this Skill?</div>
        </Modal>
      </div>
    </>
  );
