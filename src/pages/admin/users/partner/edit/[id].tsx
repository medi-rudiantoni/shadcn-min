...
function EditPartner() {
  ...
  useEffect(() => {
    if (id) {
      getPartner(id, token).then((res) => {
        if (res.data.success) {
          res.data.partner.province_id =
            res.data.partner?.province?.province_id;
          res.data.partner.city_id = res.data.partner?.city?.city_id;
          res.data.partner.subdistrict_id =
            res.data.partner?.subdistrict?.subdistrict_id;
          setPartner(res.data.partner);
          setPostalCode(res.data.partner.postalCode);
          setLoading(false);
          setSubmit(false);
          setImageUrl(
            `${process.env.NEXT_PUBLIC_IMAGE_ENDPOINT}/partners/${res.data.partner?.companyPhoto}`,
          );
        }
      });
    }
  ...
  useEffect(() => {
    if (!loading) {
      ...
      getCities(partner?.province?.province_id, token).then((res) => {
        setCities(res.data);
      });
      getSubdistricts(partner?.city?.city_id, token).then((res) => {
  ...
  const onChangeProvince = (value: any) => {
    setPartner({ ...partner, province_id: value });
    getCities(value, token).then((res) => {
      setCities(res.data);
    });
  };
  const onChangeCity = (value: any) => {
    setPartner({ ...partner, city_id: value });
    getSubdistricts(value, token).then((res) => {
      setSubdistricts(res.data);
    });
  };
  const onChangeSubdistrict = (value: any) => {
    setPartner({ ...partner, subdistrict_id: value });
    setSelecteValue(value);
  };
  const onSubmit = () => {
    setSubmit(true);
    updatePartner(id, partner, token).then((res: any) => {
      if (res.data.success) {
