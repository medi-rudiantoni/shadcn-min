...
import { getSubCategories, removeSubCategory } from "@/functions/subcategory";
...
function SubcategoriesList() {
  ...
  const handleRemove = (id: any) => {
    ...
    removeSubCategory(id, token)
      ...
      .catch((error) => {
        console.error("Remove sub-category error: ", error);
        const message =
  ...
  subcategory.map((user: User) => {
    const { _id, name, subcategory_id, createdAt } = user;
    ...
    return usersTableData.push({
      ...
      action: (
        <div className="min-w-[150px] text-end -m-2">
          ...
          <Link
            className="inline-block m-2"
            href={`/admin/subcategories/edit/${_id}`}
          >
  ...
  return (
    <>
      ...
      <div className="min-h-[715px] lg:min-h-[580px] flex-1 h-auto px-8 xl:px-[15px] pb-[30px] bg-transparent">
        ...
        >
          <div>Are you sure want to Delete this Sub-category?</div>
        </Modal>
      </div>
    </>
  );
