...
function DragAndDropTable() {
  ...
  users.map((user: User, index: number) => {
    ...
    return usersTableData.push({
      ...
      user: (
        <div className="flex items-center gap-x-[10px] gap-y-[10px]">
          <figure className="mb-0">
            <Image style={{ width: "40px" }} src={`/${img}`} alt="" />
          </figure>
          <figcaption>
            <Heading
              className="user-name text-dark dark:text-white/[.87] text-[14px] font-semibold mb-0"
              as="h6"
            >
              {name}
            </Heading>
      ...
      email: (
        <span className="text-sm font-medium text-light dark:text-white/60">
          john@gmail.com
        </span>
      ),
      company: (
  ...
  function handleDragEnd(event: any) {
    ...
    if (active && over && active.id !== over.id) {
      const activeIndex = dataSource.findIndex(
        (item: { key: string }) => item.key === active.id.key,
      );
      const overIndex = dataSource.findIndex(
        (item: { key: string }) => item.key === over.id.key,
      );
