...
import { getAllCustomers } from "@/functions/customer";
...
function CustomerList(props: Props) {
  ...
  const [admin, setCustomer] = useState(props.initialCustomers);
  ...
  useEffect(() => {
    ...
    loadCustomers(newPage, newSize, newSearch, newSort);
  ...
  function handleChangePage(newPage: number, newPageSize: number) {
    ...
    loadCustomers(newPage, newPageSize);
  }
  ...
  admin.map((user: User) => {
    const { _id, id, companyName, role, img, status, email, created_at } = user;
          ...
          <Col xs={24} className="mb-[25px]">
            <div className="w-full h-fit flex justify-between items-center">
              <Button type="primary" className="mb-3">
                <Link href="/admin/users/partner/add">Add Customer</Link>
              </Button>
