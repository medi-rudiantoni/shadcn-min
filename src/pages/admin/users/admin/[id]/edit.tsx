...
import { getAdmin, updateDataAdmin } from "@/functions/admin";
...
function Editadmin() {
  ...
  const verifyPassword = () => {
    console.log("NEW  PASS", password, "VERIF PASS", oldPassword);
  ...
  const onSubmit = async () => {
    ...
    if (password && !oldPassword) {
      toast.error("Current Password is required when setting a new password.", {
        position: "top-right",
        autoClose: 5000,
        theme: "light",
        transition: Bounce,
    ...
    const res = await updateDataAdmin(id, payload, token)
      .then(() => {
                    ...
                    <Form.Item name="oldPassword" label="Current Password">
                      <Input
                        ...
                        className={`${
                          ...
                          "border-red-800 bg-red-50 ring-1 ring-red-500 text-red-600 placeholder:text-red-600"
                        }`}
